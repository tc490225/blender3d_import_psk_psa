...
def pskimport(filepath,
    ...
    #Uncomment to move each object on import.. to prevent clumping multiple objects
    #xx = random.randrange(-500, 500)
    #yy = random.randrange(-500, 500)
    #bpy.data.objects[mesh_obj.name].location.x = xx
    #bpy.data.objects[mesh_obj.name].location.y = yy

    try:
        for mat in mesh_obj.data.materials:
            #print(mat.name)
            materialName = mat.name
            matFileNamePath = folderpath + '\\' + materialName + '.mat'
            
            with open(matFileNamePath, 'r') as searchfile:
                for line in searchfile:
                    if 'Diffuse=' in line:
                        line = line.strip()
                        imgFileName = (line.split('=')[1]) + '.png'
                        imgFileNamePath = folderpath + '\\' + imgFileName
                        ...
                        material = bpy.data.materials[mat.name]                            
                        material.use_nodes = True
                        normal_map_path = imgFileNamePath
                        nodes = material.node_tree.nodes
                        links = material.node_tree.links
                        principled_bsdf = nodes.get("Principled BSDF")
                        normal_tex = nodes.new('ShaderNodeTexImage')
                        normal_tex.location = (-600.0, 0.0)
                        normal_tex.image = bpy.data.images.load(normal_map_path)
                        normal_tex.image.colorspace_settings.name = "Non-Color"
                        normal = nodes.new('ShaderNodeNormalMap')
                        normal.location = (-300.0, 0.0)
        ...
        print("     Error finding .mat file")
    
    #toggle automatically pack into .blend
    #bpy.ops.file.autopack_toggle()

    #automatically create .blend file
    #fileName = (filename.split('.')[0]) + '.blend'
    #fileNamePath = folderpath + '\\' + fileName
    #bpy.ops.wm.save_as_mainfile(filepath=fileNamePath)
